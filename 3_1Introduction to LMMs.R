...
plot(x, xlab = "Task", ylab = "log Reaction Time (ms)", main = "Partial Effect for Task x Congruence",
...
lmer.0<-lmer(log(RTms) ~ (1|SubNo), data = Mydata.corr.a, REML = FALSE)
summary(lmer.0)

# add predictors of trial and block

lmer.1<-lmer(log(RTms) ~ Block + Trial + (1|SubNo), data = Mydata.corr.a, REML = FALSE)
summary(lmer.1)
...
lmer.0<-lmer(log(RTms) ~ (1|SubNo), data = Mydata.corr.a, REML = TRUE)
summary(lmer.0)

lmer.1<-lmer(log(RTms) ~ (1|SubNo) + (1|HomLocation), data = Mydata.corr.a, REML = TRUE)
summary(lmer.1)
